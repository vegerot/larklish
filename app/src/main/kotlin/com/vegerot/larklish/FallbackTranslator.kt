...
/**
 * [primary] first, retried once; [fallback] when that fails too. Fallback output gets a
 * leading `~` so the Relay and `events.jsonl` show which engine produced it (Layer 4).
 * Every fallback reports its reason to [onFallback], so the record can tell an outage from
 * an input Lark will not translate.
 */
class FallbackTranslator(
    ...
    override suspend fun zhToEn(text: String): String = try {
        ...
        giveUp(text, e) // deterministic: Lark returns Latin-heavy input unchanged (Experiment 05)
    } catch (e: Exception) {
...
fun defaultTranslator(onFallback: (String) -> Unit = {}): Translator =
    FallbackTranslator(LarkApiTranslator(BuildConfig.LARK_APP_ID, BuildConfig.LARK_APP_SECRET), MlKitTranslator(), onFallback)
