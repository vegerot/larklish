...
/**
 * A Lark Preview split into its parts. Lark posts `text` as `Sender: message` (Experiment 00); a
 * mention makes it `Sender@you: message` or `Sender@all: message`. The Sender never goes through
 * the Translator (Experiment 02).
 */
data class Preview(val sender: String, val mention: Mention?, val message: String) {
    ...
    val stem: String
        get() = message.removeSuffix(ELLIPSIS)

    /**
     * Lark cut the message, so the Full text holds more than this Preview does. When it did not
     * cut, the two are the same and there is nothing to fetch (Experiment 13).
     */
