...
/**
 * Lark can deliver a backlog minutes late — 3 m 59 s observed, three Originals in two seconds. Only
 * a full-length needle may reach that far back: it names one message on its own, so the extra reach
 * cannot mistake a neighbour for it.
 */
private const val BEFORE_DELAYED_MS = 300_000L

/**
 * clock slack between the phone and Lark. No message ever arrives after its Original (Experiment
 * 11).
 */
private const val AFTER_MS = 5_000L

/**
 * enough characters to name one message inside the window; 0 false positives in 35 chats
 * (Experiment 11)
 */
...
sealed interface Pick {
    ...
    /**
     * `no-chat`, `no-message` (nothing in the window), `no-match`, `type:<msg_type>`, or `error: …`
     * when the chat could not be read — recorded so the soak shows why.
     */
...
/**
 * The newest message with text just before the Original whose text opens with the Preview stem.
 * Only the first [MATCH_CHARS] folded characters must agree: the Preview is not a faithful prefix
 * of the Full text — it writes `[Delighted]` for an emoji, lower-cases `@All`, and can drop a
 * trailing bold run (Experiment 11). A message that opens with a mention is matched on the close of
 * the stem instead, because Lark localizes the name. A textless message in the window is reported
 * by type (`image`, `sticker`, `interactive`, …).
 *
 * How far back the window reaches depends on how much needle there is. A full-length one identifies
 * the message by itself, so it may look past a delayed push; a short or empty stem — `Sender:...`
 * with no boundary in 45 characters — leans on the window to tell messages apart, so it stays close
 * to the Original.
 */
fun pickMessage(items: List<Candidate>, stem: String, whenMs: Long): Pick {
    ...
    val window = items.filter {
        !it.deleted && it.createTime in (whenMs - before)..(whenMs + AFTER_MS)
    }
    window
        .firstOrNull { it.text.isNotEmpty() && it.text.fold().startsWith(needle) }
        ?.let {
            return Pick.Found(it)
        }
    ...
    if (folded.startsWith("@") && folded.length >= MATCH_CHARS) {
        ...
        window
            .firstOrNull { it.text.startsWith("@") && it.text.fold().contains(close) }
            ?.let {
                return Pick.Found(it)
            }
    }
...
/**
 * How a stem and a message text are compared: no whitespace, no case (Lark's Preview keeps
 * neither).
 */
private fun String.fold() = filterNot { it.isWhitespace() }.lowercase()

/**
 * `@_user_1` placeholders → `@Name`, from the message's `mentions` (key → name). `@_all` has no
 * entry there and reads `@all` in the Preview, so it is spelled that way here too.
 */
 ...
 * [pickMessage]. Groups: title → `chats/search`. DMs: `messages/search` around the Original's time,
 * the p2p hit whose peer name is the Sender. Chat ids are cached in [cacheFile] (`title` for
 * groups, `dm:<Sender>` for DMs). A chat that cannot be read is just one chat's miss ([pickIn]); a
 * failed *search* still throws, having no candidates to fall back on.
 */
class MessageFetcher(private val token: UserToken, private val cacheFile: File) {
    ...
    suspend fun fullTextOf(title: String, preview: Preview, whenMs: Long): Pick =
        withContext(Dispatchers.IO) {
            ...
            // search can ever find it (Experiment 09). Such a reply follows other traffic in the
            // same
    ...
    /**
     * One chat's answer. A failure here is **this chat's** answer, not the lookup's: `chats/search`
     * returns chats Max can find but not read (`230002 Bot/User can NOT be out of the chat`), and
     * one of those used to abort `fullTextOf` outright — skipping the other candidates and the LRU
     * fallback added for exactly this kind of miss (progress.md 2026-08-31). The reason still
     * reaches the record through [Pick.Skipped].
     */
        ...
        try {
            val items =
                LarkHttp.getJson(
                        ...
                        // Bound the request by the same window [pickMessage] uses, so a burst in a
                        // busy
            ...
            Log.i(
                TAG,
                "[$title] $chatId candidates: " +
                    items.joinToString { "${it.msgType} ${(whenMs - it.createTime) / 1000}s ago" },
            )
    ...
    /**
     * A DM Original has title `Lark` — bots and people alike (Experiment 09) — or, in theory, the
     * Sender's own name; everything else is a group. The two caches never cross: the bot that DMs
     * Max also posts in groups.
     */
    private fun isDm(title: String, preview: Preview) =
        title == BOT_DM_TITLE || title == preview.sender
    ...
    /**
     * Lark truncates long titles with `...`; then every group whose name starts with the stem, else
     * the exact name.
     */
    private fun groupChatIds(title: String): List<String> {
        ...
        val hits =
            LarkHttp.getJson(
                    ...
                    mapOf("query" to stem, "page_size" to "20"),
                    token.bearer(),
                )
                .getJSONObject("data")
                .getJSONArray("items")
        ...
        val match: (String) -> Boolean =
            if (title.endsWith("...")) { n -> n.startsWith(stem) } else { n -> n == title }
        return hits
            .filter { match(it.getString("name")) }
            .take(PREFIX_TRIES)
            .map { it.getString("chat_id") }
    }

    /**
     * `messages/search` (user identity) around the Original's time; the p2p hit whose peer name
     * (first line of `display_info`) is the Sender. `chat_type` is ignored without a `query`, so
     * filter on `is_p2p_chat`. Polls because the index lags ~15 s.
     */
    private suspend fun dmChatId(preview: Preview, whenMs: Long): String? {
        ...
        repeat(SEARCH_TRIES) { attempt ->
            ...
            val hits =
                LarkHttp.postJson("/open-apis/im/v1/messages/search", body, token.bearer())
                    .getJSONObject("data")
                    .getJSONArray("items")
                    .objects()
            hits
                .firstOrNull {
                    it.getJSONObject("meta_data").getBoolean("is_p2p_chat") &&
                        peerName(it) == preview.sender
                }
                ?.let {
                    return it.getJSONObject("meta_data").getString("chat_id")
                }
            Log.i(
                TAG,
                "dm search for [${preview.sender}]: ${hits.size} hits, no peer match (try ${attempt + 1})",
            )
        }
        return null
    }
...
private fun candidateOf(m: JSONObject): Candidate {
    ...
    val deleted =
        m.optBoolean(
            "deleted"
        ) // a recalled message stays in the list as "This message was recalled"
    val mentions =
        m.optJSONArray("mentions")?.objects().orEmpty().associate {
            it.getString("key") to it.getString("name")
        }
    val raw =
        when {
            deleted -> ""
            type == "text" ->
                unwrapHtml(
                    JSONObject(m.getJSONObject("body").getString("content")).getString("text")
                )
            type == "post" -> postText(JSONObject(m.getJSONObject("body").getString("content")))
            else -> ""
        }
    return Candidate(
        m.getString("message_id"),
        type,
        m.getString("create_time").toLong(),
        deleted,
        resolveMentions(raw, mentions),
    )
}
...
/**
 * A `post` is `title` + paragraphs of tagged elements (Experiment 08): `{"title": "", "content":
 * [[{"tag": "text", "text": "…"}], [{"tag": "img", "image_key": "…"}]]}`. One line per paragraph;
 * images become `[image]` and emoji `[Delighted]`, like Lark's own Preview. Pure, so the replay
 * harness scores the shipped rules instead of a copy of them.
 */
...
private fun postText(post: JSONObject): String =
    postText(
        ...
        post
            ...
            .let { ps -> (0 until ps.length()).map { ps.getJSONArray(it) } }
            .map { p ->
                p.objects().map {
                    PostElement(
                        it.getString("tag"),
                        it.optString("text"),
                        it.optString("user_name"),
                        it.optString("emoji_type"),
                    )
                }
            },
    )
