...
| A copied seed expires early | The refresh token `tools/larklish-helper token` copies is spent as soon as *that* machine's lark-cli refreshes (days): the phone then gets `authen/v2/oauth/token` 400 `20064 … can only be used once` and, because the seed changed, ignores its own `user-token.json` | `docs/progress.md` 2026-09-02 |
