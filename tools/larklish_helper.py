...
ChatCache = dict[
    str, str
]  # chats.json (MessageFetcher.kt): title or `dm:<Sender>` → chat id
...
def read_chat_cache(path: str | None = None) -> ChatCache:
    ...
    if path:
        ...
        raw = adb(
            "exec-out", "run-as", PACKAGE, "cat", "files/chats.json", allow_fail=True
        ).stdout
    raw = raw.strip()
...
def main() -> None:
    ...
    b.add_argument(
        "--debug",
        metavar="HOOK",
        help="run a MainActivity hook: user, refresh, fetch or thread",
    )
    ...
    c.add_argument(
        "--clear", action="store_true", help="delete the cache and force-stop the app"
    )
    ...
    fetch.add_argument(
        "--chats", help="read this saved chats.json instead of the phone (DM senders)"
    )
    r.set_defaults(fn=cmd_replay)

    t = group.add_parser(
        "token", help="credentials from lark-cli's store (runs tools/lark-token)"
    )
    t.add_argument(
        "rest", nargs=argparse.REMAINDER, help="passed through, e.g. --write"
    )
