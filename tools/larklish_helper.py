...
Event = dict[
    str, Any
]  # one line of events.jsonl (Recorder.kt): `event`, `at`, `key`, …
Relay = dict[
    str, Any
]  # an Event of kind `relayed`, plus `outcome` and `cut` (relays_with_outcomes)
Message = dict[
    str, Any
]  # one item of im/v1/messages: `msg_type`, `create_time`, `body.content`, …
ChatCache = dict[
    str, str
]  # the Backend's chat cache (GET /chats): title or `dm:<Sender>` → chat id
...
def need(module: str) -> Any:
    ...
    try:
        ...
        sys.exit(
            f"{module} is not installed for this python — run the helper in the repo's venv:\n"
...
def is_fallback(e: Event) -> bool:
    ...
    return e["relayTitle"].startswith("~") or e["relayText"].split(": ", 1)[
        -1
    ].startswith("~")
...
def event_ms(e: Event) -> int:
    return int(
        datetime.fromisoformat(e["at"].replace("Z", "+00:00")).timestamp() * 1000
    )
...
TEST_TITLE_EN = (
    "Larklish test group"  # the Relay's title: Lark's translation of TEST_TITLE
)


def lark_cli(
    method: str, path: str, payload: dict[str, Any], identity: str = "user"
) -> dict[str, Any]:
...
def resolve_chat(name: str) -> tuple[str, str]:
    ...
    hits = [
        h for h in chats_search(stem) if names_chat(h["name"], name, stem)
    ] or chats_search(stem)
    if not hits:
...
def flat_text(m: Message) -> str:
    ...
    if kind == "interactive":
        try:
            title = json.loads(body).get("title") or json.loads(body).get(
                "header", {}
...
def store_dir() -> pathlib.Path:
    ...
    return (
        pathlib.Path(
            os.environ.get(
                "LARKSUITE_CLI_DATA_DIR", pathlib.Path.home() / ".local/share"
            )
        )
        / "lark-cli"
    )
...
def write_local(values: dict[str, str]) -> None:
    ...
    lines = (
        LOCAL_PROPERTIES.read_text().splitlines() if LOCAL_PROPERTIES.exists() else []
    )
...
SHADE_BOTTOM = (
    2261  # the shade's scroller ends here on the Pixel 4a (below it: Clear all)
)
Node = ET.Element
...
def bounds(node: Node) -> Bounds:
    left, top, right, bottom = (
        int(x) for x in node.get("bounds", "").replace("][", ",").strip("[]").split(",")
    )
...
def header_button(group: Node) -> Node | None:
    ...
    for n in group.iter("node"):
        if (
            n.get("class") == "android.widget.Button"
            and bounds(n)[1] < bounds(group)[1] + 200
        ):
            return n
    return None
...
def locate(app: str, title: str) -> tuple[Node, Node]:
    ...
    for _ in range(5):
        ...
        header = find_node(
            root, **{"resource-id": "android:id/app_name_text", "text": app}
        )
        chevron = (
            button(rows_of(header, parent)[-1], "Expand")
...
def rounded(card: Any) -> Any:
    ...
    ImageDraw.Draw(mask_).rounded_rectangle(
...
def compose(slug: str, caption: str) -> str:
    ...
    page = Image.new(
        "RGB",
        (
            PAD + lark.width + GAP + larklish.width + PAD,
            top + max(lark.height, larklish.height) + PAD,
        ),
        PAGE_BG,
    )
    ...
    d.text(
        (PAD + lark.width + GAP, PAD + 70),
        "What Larklish showed",
        fill=LABEL,
        font=small,
    )
...
def events_list(events: list[Event], args: argparse.Namespace) -> None:
    for e in selected(events, args):
        ...
        if kind == "relayed":
            ...
            r = e["reason"]
            print(
                f"{when(e, args.utc)} X  reason={r} ({REASONS.get(r, '?')}) chat {event_chat(e)}"
            )
...
def events_stats(events: list[Event], args: argparse.Namespace) -> None:
    ...
    print(
        f"relayed {len(relayed)}  updated {len(updated)}  skipped {len(skipped)}  removed {len(removed)}"
    )
    ...
    print(
        ...
        ", ".join(
            f"{r} ({REASONS.get(r, '?')}) ×{n}"
            for r, n in sorted(Counter(e["reason"] for e in removed).items())
        ),
    )
    if relayed:
...
def cmd_msgs(args: argparse.Namespace) -> None:
    ...
    start, end = (
        int(around - parse_span(args.before)),
        int(around + parse_span(args.after)),
    )
    ...
    for m in sorted(items, key=lambda m: int(m["create_time"])):
        ...
        if not args.utc:
            t = t.astimezone()
        mentions = " ".join(
            f"{x['key']}={x['name']}" for x in (m.get("mentions") or [])
        )
        line = m["body"]["content"] if args.raw else one_line(flat_text(m))
        print(
            f"{t.strftime('%m-%d %H:%M:%S')}  {m['msg_type']:12} {line}"
...
def cmd_translate(args: argparse.Namespace) -> None:
    ...
    for i in range(args.repeat):
        ...
        d = lark_cli(
            ...
            {"source_language": "zh", "target_language": "en", "text": args.text},
...
def cmd_replay(args: argparse.Namespace) -> None:
    ...
    for e in relayed:
        ...
        ids = [
            ...
            if names_chat(h["name"], title, stem)
        ][:5]
        titles[title] = ids
        chats.update(ids)
...
def cmd_showcase(args: argparse.Namespace) -> None:
    ...
    if not args.no_send:
        ...
        time.sleep(
            14
        )  # Relay in ~1 s, the Update through the Backend a few seconds later
    adb("shell", "input", "keyevent", "KEYCODE_HOME")
    time.sleep(1)
    adb("shell", "cmd", "statusbar", "expand-notifications")
    time.sleep(2.5)
    for app, title, half in (
        ("Lark", TEST_TITLE, "lark"),
        ("Larklish", TEST_TITLE_EN, "larklish"),
    ):
...
def main() -> None:
    ...
    e = group.add_parser(
        "events",
        help="the Relay record",
        description="Views of `filesDir/events.jsonl`.",
    )
    ...
    b.add_argument(
        "--thread",
        action="store_true",
        help="post a root message, then reply inside its thread",
    )
    b.add_argument(
        "--debug",
        metavar="HOOK",
    ...
    c = group.add_parser(
        "chats", help="the Backend's chat-id cache; `chats search` asks the API"
    )
    ...
    cs.add_argument(
        ...
        type=int,
        default=1,
        help="ask N times; shows in how many runs each hit appeared",
    )
    cs.set_defaults(fn=cmd_chats_search)

    m = group.add_parser(
        "msgs", help="what a chat said around a time (im/v1/messages, flattened)"
    )
    m.add_argument(
        "chat",
        help="oc_… id, or a title — resolved through the phone's cache, then chats/search",
    )
    m.add_argument(
        "--around", help="ISO time, local unless it ends in Z (default: now)"
    )
    m.add_argument(
        "--before", default="10m", help="reach back this far: 90s, 5m, 2h (default 10m)"
    )
    m.add_argument("--after", default="1m", help="and forward this far (default 1m)")
    m.add_argument(
        "--raw", action="store_true", help="print body.content as the API returns it"
    )
    m.add_argument("--utc", action="store_true", help="print UTC instead of local time")
    m.set_defaults(fn=cmd_msgs)

    x = group.add_parser(
        "translate", help="one call to Lark's translate API, with its code"
    )
    x.add_argument("text")
    x.add_argument(
        "--repeat",
        type=int,
        default=1,
        help="call N times and tally the codes (the rate-limit probe)",
    )
    x.add_argument(
        "--pause", type=float, default=0, help="seconds between calls (default 0)"
    )
    x.add_argument(
        "--as",
        dest="identity",
        default="bot",
        choices=["bot", "user"],
        help="identity (default bot, as the app)",
    )
    ...
    act.add_parser("shade", help="expand the shade, optional screenshot").add_argument(
        "file", nargs="?"
    )
    ...
    fetch = rsub.add_parser(
        "fetch", help="pull Originals, chats and messages into a corpus directory"
    )
    fetch.add_argument(
        "--out",
        default="replay-corpus",
        help="corpus directory (default replay-corpus/)",
    )
    fetch.add_argument(
        "--file", help="read this saved events.jsonl instead of the phone"
    )
    fetch.add_argument(
        "--chats", help="read this saved chats.json instead of the Backend (DM senders)"
    )
    r.set_defaults(fn=cmd_replay)

    s = group.add_parser(
        "showcase",
        help="post one message; crop Lark's Original and Larklish's Relay side by side into docs/showcase/",
    )
    s.add_argument("slug", help="file name stem, e.g. 02-english")
    s.add_argument(
        "text",
        help='the message to post; <at user_id="ou_…">Name</at> mentions someone',
    )
    s.add_argument("caption", help="one line above the pair")
    s.add_argument(
        "--no-send", action="store_true", help="the pair is in the shade already"
    )
    s.set_defaults(fn=cmd_showcase)

    t = group.add_parser(
        "token",
        help="credentials from lark-cli's store (masked); --write puts them in local.properties",
    )
    t.add_argument(
        "--write",
        action="store_true",
